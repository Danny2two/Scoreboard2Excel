...
file = nbt.read_from_nbt_file("scoreboard.dat") #Read Data, pointed to a scoreboard.dat normaly
ExcelOutputName = "Playerdata.xlsx" #Export file name, must end with .xlsx

Search_Terms_List = ["ts_Jump", "ts_Walk"] #Give the names of the objectives we want to pull
...
for term in Search_Terms_List:
    ...
    if (inp := input("Export to Excel Sheet? \ny for yes other key for no \n") == "y"):
        namelist = []
        scorelist = []
        ...
        if namelist != []: #check that some names where found
            ...
            if not exists(ExcelOutputName):
                print("Error, Expected file "+ ExcelOutputName + " was not found! Perhaps the export failed somehow.")
            else:
                print("File " + ExcelOutputName + " was created succesfully. \n")
        else:
